...
# png(file="exploratory1.png", width=600, height=400)
# hist(log(df.sev$ClaimAmount), breaks=500,
#      xlab="log(Claim Amount)", main="Histogram of log(Claim Amount)")
# dev.off()
# 
# png(file="exploratory2.png", width=600, height=400)
# hist(log(df.all$ClaimAmount), breaks=500,
#      xlab="log(Aggregate Amount)", main="Histogram of log(Aggregate Amount)")
# dev.off()
```
...
Y = matrix(c(rep(0, sample.size), # = tl
             df.all$AggreAmount, # = yl
             df.all$AggreAmount, # = yu
             rep(Inf, sample.size) # = tu
             ), ncol = 4, byrow = FALSE)
```
...
df.all.norm$AggreAmount = (df.all$AggreAmount - mean(df.all$AggreAmount))/sd(df.all$AggreAmount)
...
wss <- sapply(1:10,
              function(k){kmeans(data.matrix(df.freq.norm), k)$tot.withinss})

png(file="wss_freq.png", width=600, height=400)
plot(1:10, wss, main="Elbow Method for Frequency",
...
set.seed(7777)
init.analysis.3 = CMMSeverity(Y[,2], X, 3)
save(init.analysis.3, file = "Init3.Rda")

set.seed(7777)
init.analysis.4 = CMMSeverity(Y[,2], X, 4)
save(init.analysis.4, file = "Init4.Rda")

set.seed(7777)
init.analysis.5 = CMMSeverity(Y[,2], X, 5)
save(init.analysis.5, file = "Init5.Rda")

set.seed(7777)
init.analysis.6 = CMMSeverity(Y[,2], X, 6)
save(init.analysis.6, file = "Init6.Rda")
```

```{r Init analysis - frequency}
set.seed(7777)
init.analysis.4.freq = CMMFrequency(Y.freq[,2], X, 4)
save(init.analysis.4.freq, file = "freq.Init4.Rda")

set.seed(7777)
init.analysis.5.freq = CMMFrequency(Y.freq[,2], X, 5)
save(init.analysis.5.freq, file = "freq.Init5.Rda")

set.seed(7777)
init.analysis.6.freq = CMMFrequency(Y.freq[,2], X, 6)
save(init.analysis.6.freq, file = "freq.Init6.Rda")
```

```{r Init analysis - severity}
set.seed(7777)
init.analysis.4.sev = CMMSeverity(Y.sev[,2], X.sev, 4)
save(init.analysis.4.sev, file = "sev.Init4.Rda")

set.seed(7777)
init.analysis.6.sev = CMMSeverity(Y.sev[,2], X.sev, 6)
save(init.analysis.6.sev, file = "sev.Init6.Rda")

set.seed(7777)
init.analysis.8.sev = CMMSeverity(Y.sev[,2], X.sev, 8)
save(init.analysis.8.sev, file = "sev.Init8.Rda")
```
